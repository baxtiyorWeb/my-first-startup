...
export default function BookmarksPage() {
  ...
  const handleSaveChange = async (postId: string, isSaved: boolean) => {
    ...
    try {
      ...
      const msg =
        err instanceof ApiError ? err.message : t("feed.saveError");
      toast.error(msg);
      fetchBookmarks();
    }
  };
        ...
        <div className="text-center py-14 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
          ...
          <div className="mt-4">
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-slate-950 hover:bg-slate-800 text-white dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200 text-xs font-semibold cursor-pointer transition-colors"
            >
              <span>{t("bookmarks.browseFeed")}</span>
            </Link>
          </div>
        </div>
      ) : (
