...
export default function OnboardingPage() {
  ...
  const handleStep1Next = () => {
    ...
    if (!handle.trim()) {
      setError(t("onboarding.handleRequired"));
      return;
    }
    if (!role.trim()) {
      setError(t("onboarding.roleRequired"));
      return;
    }
    setError(null);
    setStep(2);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 py-8 px-4 sm:px-6 flex flex-col justify-center items-center relative">
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mt-6 text-center text-xs text-slate-400 dark:text-slate-500 flex items-center justify-center gap-2">
          ...
          <span>{t("onboarding.footerNote")}</span>
        </div>
      </div>
    </div>
  );
