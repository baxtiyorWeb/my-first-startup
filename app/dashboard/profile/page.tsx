...
function ProfileContent() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof ApiError ? err.message : "Profilni yuklashda xatolik yuz berdi";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  // Active discussions based on comment count
  const activeDiscussions = useMemo(() => {
    return userPosts.filter((p) => p.commentsCount >= 1);
  }, [userPosts]);

  // Featured thought (first post or highest engaged)
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      const msg =
        err instanceof ApiError ? err.message : "Kuzatish amalida xatolik yuz berdi";
      toast.error(msg);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 w-full animate-pulse">
        <div className="h-44 bg-slate-100 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-800" />
        <div className="h-32 bg-slate-100 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-800" />
      </div>
    );
  }
    ...
    return (
      <div className="py-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 space-y-3">
        <p className="text-sm font-semibold text-rose-600 dark:text-rose-400">
          {error || "Muallif profili topilmadi"}
        </p>
        <button
          type="button"
          onClick={() => {
    ...
    <div className="space-y-4 w-full">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-t-lg px-2 pt-1 shadow-2xs">
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-none">
          ...
          >
            Faol munozaralar ({activeDiscussions.length})
          </button>
        </div>
      </div>
            ...
            {userPosts.length === 0 ? (
              <div className="py-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                  Hozircha fikrlar chop etilmagan
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  Ushbu muallif hali platformada yangi fikrlar ulashmagan.
                </p>
              </div>
            ) : (
              userPosts.map((post) => (
            ...
            {activeDiscussions.length === 0 ? (
              <div className="py-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
                ...
                <p className="mt-3 text-sm font-semibold text-slate-800 dark:text-slate-200">
                  Hozircha faol munozaralar yo‘q
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  Muallif ishtirok etgan barcha qizg‘in bahslar shu yerda jamlanadi.
                </p>
              </div>
            ) : (
