...
function PostDetailInner({
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    if (!session.isAuthenticated) {
      toast.info("Fikr bildirish uchun tizimga kiring");
      return;
    }
    ...
    try {
      ...
      setNewCommentText("");
      toast.success("Fikringiz bildirildi");
    } catch {
  ...
  const handleAddReply = async (parentId: string) => {
    ...
    if (!session.isAuthenticated) {
      toast.info("Javob yozish uchun tizimga kiring");
      return;
    }
    ...
    try {
      ...
      toast.error("Javob yozishda xatolik yuz berdi");
    } finally {
  ...
  return (
    <div className="w-full py-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/dashboard"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors py-1.5 px-2.5 -ml-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
        >
          <ArrowLeft size={15} />
          <span>Fikrlar oqimiga qaytish</span>
        </Link>
      </div>
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link
              href={`/dashboard/profile?user=${encodeURIComponent(post.author.handle)}`}
        ...
        <div className="mt-5 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-slate-500 dark:text-slate-400">
          <div className="flex items-center gap-5 sm:gap-7">
            ...
            >
              ...
              <span className="hidden sm:inline-block text-[11px] text-slate-400">ko‘rish</span>
            </div>
        ...
        <form onSubmit={handleAddComment} className="space-y-3">
          <textarea
            ...
            placeholder={
              session.isAuthenticated
                ? "Fikrga o‘z tahlilingiz yoki mulohazangizni bildiring..."
                : "Munozarada qatnashish uchun avval tizimga kiring"
            }
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-slate-400">
              Hurmat va intellektual darajani saqlang
            </span>
            <button
              type="submit"
      ...
      <section className="space-y-3">
        ...
        {comments.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 text-center text-slate-400 dark:text-slate-500 text-xs">
            Hozircha izohlar yo‘q. Birinchi bo‘lib o‘z fikringizni bildiring!
          </div>
        ) : (
          comments.map((comment) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 text-xs text-slate-400">
                ...
                >
                  {replyingToId === comment.id ? "Bekor qilish" : "Javob yozish"}
                </button>
              </div>
              ...
              {replyingToId === comment.id && (
                <div className="pl-4 border-l-2 border-slate-200 dark:border-slate-800 space-y-2 mt-2">
                  <textarea
                    ...
                    placeholder={`@${comment.author.handle.replace("@", "")} ga javob yozish...`}
                    rows={2}
      ...
      <ConfirmDialog
        ...
        description="Ushbu fikrni o‘chirib tashlamoqchimisiz? Bu amalni ortga qaytarib bo‘lmaydi."
